...
[섯다게임은 두 장의 카드의 숫자를 더한 값을 10으로 나눈 나머지가 높은 쪽이 이기는 게임이다 
...
class SutdaDeck {
	...
	void registerJokbo() {
		...
		/*
		 * (1) . 아래의 로직에 맞게 코드를 작성하시오 1. jokbo(HashMap) . 에 족보를 저장한다 두 카드의 값을
		 * 문자열로 붙여서 로 점수를 로 저장한다
		 */
	...
	int getPoint(Player p) {
		 ...
		 * 
		 * 1. 카드 두 장이 모두 광이면 , jokbo에서 "KK"키를 로해서 점수를 조회한다 
		...
		if( jokbo.get(c1.toString()+c2.toString()) !=null){
			result = (Integer) jokbo.get(c1.toString()+c2.toString());
		}
		else{
